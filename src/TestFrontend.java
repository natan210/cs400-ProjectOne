...
public class TestFrontend {
	...
	public void runTests() {
		...
		System.out.print("Test 'r' load rating selection screen (WARNING: if pressing 'x' twice from "
				+ "the rating selection screen does not exit app, test won't exit and run indefinitely!):");
		if (this.testFrontendRForRatings()){
			System.out.println("PASSED");
		} else {
			System.out.println("FAILED");
		}
		System.out.print("Test '5' displaying movies with ratings from [5,6) on the rating selection "
				+ "screen (WARNING: if pressing 'x' twice from the rating selection screen does not exit app, test won't exit and run indefinitely!):");
		if (this.testFrontendGetRatings()) {
			System.out.println("PASSED");
		} else {
			System.out.println("FAILED");
		}
	...
	public boolean enterXToExit() {
		...
		try {
			...
			((Frontend)frontend).run(new BackendDummy(new StringReader(
	...
	public boolean testFrontendGForGenres() {
		...
		try {
			...
			String input = "g" + System.getProperty("line.separator") + "x"
	...
	/**
	 * This test runs the front end and redirects its output to a string. It then
	 * passes in 'r' as a command to go to the rating selection mode. It then exits
	 * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
	 * The test succeeds if the rating selection screen contains movie ratings included
	 * in the data. It fails if any of them are missing, the front end has not
	 * been instantiated (is null), or there is an exception.
	 * @return true if the test passed, false if it failed
	 */
	public boolean testFrontendRForRatings() {
		...
		InputStream standardIn = System.in;
		try {
			// set the input stream to our input (with an r to test if the program lists ratings)
			...
			// add all tests to this method
			String appOutput = outputStreamCaptor.toString();
			if (frontend != null && appOutput.contains("1")
	...
	/**
	 * This test runs the front end and redirects its output to a string. It then
	 * passes in 'r' as a command to go to the rating selection mode. It then passes
	 * in '5' to output all movies in the database with a rating within the range of
	 * [5,6). It fails if any of them are missing, the front end has not
	 * been instantiated (is null), or there is an exception.
	 * @return true if the test passed, false if it failed
	 */
	public boolean testFrontendGetRatings() {
		PrintStream standardOut = System.out;
		InputStream standardIn = System.in;
		try {
			/* set the input stream to our input (r to navigate to ratings selection mode, 5 to test if
				 the program lists movies) */
			String input = "r" + System.lineSeparator() + "5" + System.lineSeparator() + "x"
			...
			"title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
			+ "The Source of Shadows,The Source of Shadows,2020,Horror,83,USA,English,\"Ryan Bury, Jennifer Bonior\",\"Jennifer Bonior, Trevor Botkin\",Four Thieves Productions,\"Ashleigh Allard, Tom Bonington, Eliane Gagnon, Marissa Kaye Grinestaff, Jenna Heffernan, Joshua Hummel, Janice Kingsley, Chris Labasbas, Jared Laufree, Dominic Lee, Vic May, Sienna Mazzone, Lizzie Mounter, Grace Mumm, Ashley Otis\",\"A series of stories woven together by one of our most primal fears, the fear of the unknown.\",3.5\n"
			+ "The Insurrection,The Insurrection,2020,Action,90,USA,English,Rene Perez,Rene Perez,,\"Michael Paré, Wilma Elles, Joseph Camilleri, Rebecca Tarabocchia, Jeanine Harrington, Malorie Glavan, Danner Boyd, Michael Cendejas, Woody Clendenen, Keely Dervin, Aaron Harvey, Tony Jackson, Michael Jarrod, Angelina Karo, Bernie Kelly\",The director of the largest media company wants to expose how left-wing powers use film to control populations.,5.9\n"
			+ "Valley Girl,Valley Girl,2020,\"Comedy, Musical, Romance\",102,USA,English,Rachel Lee Goldenberg,\"Amy Talkington, Andrew Lane\",Sneak Preview Productions,\"Jessica Rothe, Josh Whitehouse, Jessie Ennis, Ashleigh Murray, Chloe Bennet, Logan Paul, Mae Whitman, Mario Revolori, Rob Huebel, Judy Greer, Alex Lewis, Alex MacNicoll, Danny Ramirez, Andrew Kai, Allyn Rachel\",\"Set to a new wave '80s soundtrack, a pair of young lovers from different backgrounds defy their parents and friends to stay together. A musical adaptation of the 1983 film.\",5.4\n"
			)));
			// set the output back to standard out for running the test
			System.setOut(standardOut);
			...
			if (frontend != null && appOutput.contains("The Insurrection")
				&& appOutput.contains("Valley Girl")) {
				// test passes if all movies with a rating from [5,6) are displayed on the screen
				return true;
			} else {
				// test failed
				return false;
